...
async function check() {
    ...
    if (snap.exists()) {
        ...
        if (data.menus) console.log("menus length:", data.menus.length);
        console.log("ingredients exists:", !!data.ingredients);
        if (data.ingredients) console.log("ingredients length:", data.ingredients.length);
    } else {
        console.log("restaurants/default Not exists");
    }
    process.exit(0);
}
check();
