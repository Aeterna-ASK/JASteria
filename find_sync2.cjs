...
async function processLineByLine() {
  ...
  for await (const line of rl) {
    if (line.includes('syncFromRiRyLink') && (line.includes('function') || line.includes('async ') || line.includes('const syncFromRiRyLink'))) {
       if(line.includes('USER_REQUEST') || line.includes('EPHEMERAL_MESSAGE')) continue;
